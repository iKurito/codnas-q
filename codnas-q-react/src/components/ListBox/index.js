            ...
            <Listbox.Options className="absolute w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {data.map((person, personIdx) => (
                <Listbox.Option
                  key={personIdx}
                  className={({ active }) =>
                    `${active ? 'text-amber-900 bg-primary-light' : 'text-gray-900'}
                          cursor-pointer select-none relative py-2 pl-10 pr-4 text-sm`
                  }
                  value={person}
                >
                  {({ selected, active }) => (
                    <>
                      ...
                      {selected ? (
                        <span
                          className={`${active ? 'text-primary-dark' : 'text-primary-dark'}
